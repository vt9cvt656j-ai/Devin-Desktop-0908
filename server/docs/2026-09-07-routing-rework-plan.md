...
**关键约束（文档原话）**：倍率不可为 0，除非完全免费（`CHECK (rate = 0 OR rate > 0)` 落成
`rate <> 0` 且 `rate > 0` 语义由应用层把关——0 即免费）。拉模型时若中转能报出价格就带下来
...
| `pages/Routing.tsx`（线路） | 按文档 §线路界面 重做表单：序号/站点名+base_url/使用状态/模型数量/计费方式/上次使用时间/按钮列；停用提示影响 X 路由 Y 出口；删除弹窗列受影响路由明细 |
