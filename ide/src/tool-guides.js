...
const TOOL_METADATA = Object.freeze({
  ...
  computer: { category: 'desktop_automation', use_cases: ['桌面鼠标/键盘操作', '窗口管理', '剪贴板操作'], triggers: ['操控桌面应用', '模拟用户操作', '操作非浏览器窗口'], example_call: "computer(action='screenshot') → computer(action='left_click', coordinate=[640,413])", priority: 'medium', usage_note: '【何时用】操控桌面应用的鼠标键盘/窗口。桌面专用。【vs 替代】浏览器内操作用 browser；系统菜单/应用跳转用 system。【何时不用】纯浏览器内交互用 browser。' },
  ...
  browser: { category: 'ui_automation', use_cases: ['登录/多步表单', '点击/输入/上传', 'E2E/UI 行为验证', '抓真实接口前置'], triggers: ['需要真实浏览器交互', '验证前端行为', '登录后才可见的页面'], example_call: "browser(action='navigate', url='http://127.0.0.1:5174', fresh=true)", priority: 'high', usage_note: '【何时用】需要真实浏览器交互（登录、多步表单、点击、E2E/UI 行为验证）时；默认流程 navigate→read/find（把页面当文档读、按文字定位）→按 node 操作（连续多步 batch，不知道几步用 task）→assert/check。【vs 替代】只看静态布局/最终视觉验收用 screenshot；抓纯数据用 http_request；读源码用 read_file。【何时不用】读文件或抓纯数据时，绝不用浏览器一页页抄。' },
...
const TOOL_EXAMPLES = Object.freeze({
  ...
  office_edit: { path: "reports/q3.xlsx", spec: { sheets: [{ name: "Sheet1", cells: { B2: 42 } }] } },
  office_read: { path: "reports/q3.xlsx" },
