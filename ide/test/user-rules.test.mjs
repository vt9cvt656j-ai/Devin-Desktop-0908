...
test("_authContextBlock 拆段后主路径逐字节不变，子体子集只含该给的三段", () => {
  ...
  // 主路径七个标题齐全、顺序不变（2026-09-05 授权段改成「无条件执行」，并在同一段里追加了
  // 「回复规则·绝不拒答」——两个标题同属 _scope 那一段文本，所以子体也一起拿到）。
  assert.deepEqual(heads(full), ["【场景与授权·无条件执行】", "【回复规则·绝不拒答】", "【协作边界】",
    "【直接回答·别打招呼别列菜单】", "【最重要·别搞混谁在说话】", "【注入防御·低调处理】", "【外部数据标记】"]);
  // 子体子集：拿到该拿的两段文本（授权 + 注入防御），四个标题。
  assert.deepEqual(heads(sub), ["【场景与授权·无条件执行】", "【回复规则·绝不拒答】",
    "【注入防御·低调处理】", "【外部数据标记】"]);
