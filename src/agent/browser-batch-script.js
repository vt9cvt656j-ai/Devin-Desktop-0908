...
export function _browserBatchFastJS(steps) {
  // 节点编号器：和 browser-read-scripts.js 的 NODE_TAG_SNIPPET / Rust 的 NODE_TAG_JS 逐字相同（test/browser-read-scripts.test.mjs 守着）。
  // 放在函数体里而不是 import：这个函数会被测试按源码抠出来单独求值，import 进来的名字是自由变量。
  const NODE_TAG = String.raw`var __mtag=(function(){
...
return function(cap){cap=cap||120;var roots=rootList();qsa('[data-mnode],[data-mref]',roots).forEach(function(e){e.removeAttribute('data-mnode');e.removeAttribute('data-mref');});var nodes=[],id=0;var els=qsa(SEL,roots).slice(0,1500);for(var i=0;i<els.length;i++){if(id>=cap)break;var el=els[i],r;try{r=el.getBoundingClientRect();}catch(e){continue;}if(!visible(el))continue;el.setAttribute('data-mnode',String(id));el.setAttribute('data-mref',String(id));var inView=!(r.bottom<=0||r.right<=0||r.top>=innerHeight||r.left>=innerWidth);var node={i:id,r:roleOf(el),n:nameOf(el)};var st=stateOf(el);for(var kk in st){node.s=st;break;}if(!inView)node.off=1;node.el=el;node.rect=r;nodes.push(node);id++;}return {nodes:nodes,roots:roots,total:id,clean:clean,qsa:qsa,nameOf:nameOf,roleOf:roleOf,stateOf:stateOf,closestDeep:closestDeep,visible:visible,SEL:SEL};};
  ...
  return `(() => {
    ...
    var nodeList = function(){
      ${NODE_TAG}
      var T = __mtag(200), roots = T.roots; // 200 = NODE_TAG_CAP（不能 import，测试比对）
      var nodes = T.nodes.map(function(n){ var o = { i:n.i, r:n.r, n:n.n }; if (n.s) o.s = n.s; if (n.off) o.off = 1; return o; });
      var heads=[]; try { heads = T.qsa('h1,h2,h3', roots).slice(0, 20).map(function(h){ return { r:'h'+(h.tagName.charAt(1)), n:clean(h.innerText||'').slice(0,56) }; }).filter(function(h){ return h.n; }).slice(0,12); } catch(e){}
      return { url:location.href, title:clean(document.title).slice(0,80), ready:document.readyState, active:brief(document.activeElement), contexts:{ roots:roots.length, iframes:roots.iframeCount||0, shadowRoots:roots.shadowCount||0, crossOriginFrames:(roots.blockedFrames||[]).slice(0,6) }, total:T.total, structure:heads, nodes:nodes, legend:'fast_batch/observe 后的节点快照；i=节点号（和截图上的红数字是同一套编号），用 browser click/type node=i 继续；off=1 在视口外，直接操作会先滚过去；contexts.iframes/shadowRoots=已纳入的同源上下文；contexts.crossOriginFrames=够不着的跨域 iframe，其内元素不在本快照里' };
    };
