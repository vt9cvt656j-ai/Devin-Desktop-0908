...
// 「走自己的端点会变弱」这条披露的三次搬家，和它最后被删掉。
//
// 事实本身没变，而且仍然成立：工具描述由服务端按名回填、完整系统提示词也在服务端，
// 第三方端点两头落空，智能体明显变弱；长上下文压缩也会关闭。
//
// 它出现过三个位置，每一次都是所有者嫌它占地方：
//   ① 切端点时弹的 toast（九秒横幅、切一次弹一次）→ ② 自定义模型弹窗顶部的说明
//   → ③ 模型悬浮卡的 .mic-note（2026-09-01）。
// 前两次搬家时这里都留着一条「披露不能跟着消失」的守卫，判据跟着搬。
//
// 【2026-09-01 第三次，也是最后一次】所有者要求把悬浮卡上那段也删掉，整条披露就此
// 不再存在于产品任何位置。这是他的产品决定，不是回归——所以守卫跟着撤掉，而不是
// 留一条恒真的断言在这儿守空气。
//
// 留下这段注释是为了下一个人：在别处看到「智能体走自定义端点会变弱」的痕迹时，
// 不要以为披露被谁误删了而"修复"回来。要重新加，先问所有者。
